...
class FCNSegementer(object):
    ...
    def save_model(self, sess):
        print("Saving model")
        model_file = os.path.join(self.logs_location, "model.cpkt")
        saver = tf.train.Saver()
        saver.save(sess, model_file, write_meta_graph=False)
    ...
    def train_nn(self, sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
        ...
        for epoch in range(epochs):
            ...
            for images, labels in get_batches_fn(batch_size):
                ...
                print("Batch {}...".format(batch_counter), "Training Loss: {:.5f}...".format(loss))
                total_loss.append(loss)
                batch_counter +=1
            train_loss = sum(total_loss) / (batch_counter )
            print("Epoch {} of {}...".format(epoch+1, epochs), "Training Loss: {:.5f}...".format(train_loss))
